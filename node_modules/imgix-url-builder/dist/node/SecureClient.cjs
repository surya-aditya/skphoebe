...
const buildPixelDensitySrcSet = require("../buildPixelDensitySrcSet.cjs");
const buildSignedPixelDensitySrcSet = require("./buildSignedPixelDensitySrcSet.cjs");
const buildSignedURL = require("./buildSignedURL.cjs");
const buildSignedWidthSrcSet = require("./buildSignedWidthSrcSet.cjs");
const buildURL = require("../buildURL.cjs");
const buildWidthSrcSet = require("../buildWidthSrcSet.cjs");
const signURL = require("./signURL.cjs");
class SecureClient {
  /**
  ...
  constructor(options) {
     ...
     */
    __publicField(this, "secureURLToken");
  ...
  buildURLForPath(path, params = {}) {
    return buildURL.buildURL(`${new URL(path, this.baseURL)}`, params);
  }
  /**
  ...
  buildSignedURL(url, params = {}) {
    return buildSignedURL.buildSignedURL(url, this.secureURLToken, params);
  }
  /**
   ...
   */
  signURL(url) {
    return signURL.signURL(url, this.secureURLToken);
  }
  /**
  ...
  buildSignedWidthSrcSet(url, params) {
    return buildSignedWidthSrcSet.buildSignedWidthSrcSet(url, this.secureURLToken, params);
  }
  /**
  ...
  buildWidthSrcSetForPath(path, params) {
    return buildWidthSrcSet.buildWidthSrcSet(`${new URL(path, this.baseURL)}`, params);
  }
  /**
   * Builds an `<img>` `srcset` attribute value for a given set of widths for
   * the client's base URL. It can also optinally apply Imgix URL API parameters
  ...
  buildSignedPixelDensitySrcSet(url, params) {
    return buildSignedPixelDensitySrcSet.buildSignedPixelDensitySrcSet(url, this.secureURLToken, params);
  }
  /**
   * Builds an `<img>` `srcset` attribute value for a given set of pixel
  ...
  buildPixelDensitySrcSetForPath(path, params) {
    return buildPixelDensitySrcSet.buildPixelDensitySrcSet(`${new URL(path, this.baseURL)}`, params);
  }
  /**
   * Builds an `<img>` `srcset` attribute value for a given set of pixel
   * densities for the client's base URL. It can also optinally apply Imgix URL
  ...
  buildSignedPixelDensitySrcSetForPath(path, params) {
    return buildSignedPixelDensitySrcSet.buildSignedPixelDensitySrcSet(`${new URL(path, this.baseURL)}`, this.secureURLToken, params);
  }
}
exports.SecureClient = SecureClient;
//# sourceMappingURL=SecureClient.cjs.map
